...
dl_apkmirror() {
  ...
  url="https://www.apkmirror.com$(req "$url" - | tr '\n' ' ' \
  | sed -n 's;.*href="\(.*key=[^"]*\)">.*;\1;p')"
  echo -e "${BLUE}Downloading ${RED}$output${BLUE} from ${CYAN}$url${NC}"
...
get_apkmirror() {
  ...
  if [[ -z $arch ]]; then
    echo -e "${YELLOW}Downloading ${RED}$app_name${NC}"
  else
    for i in {0..3}; do
      if [[ $arch == ${arch_array[i]} ]]; then
        echo -e "${YELLOW}Downloading ${RED}$app_name (${arch_array[i]})${NC}"
  ...
  echo -e "${YELLOW}Choosing version ${CYAN}'${version}'${NC}"
...
get_uptodown() {
    local apk_name="$1"
    local link_name="$2"
    echo -e "${YELLOW}Downloading ${RED}$apk_name${NC}"
    export version="$version"
    ...
    if [[ " ${available_versions[@]} " =~ " ${version} " ]]; then
        echo -e "${YELLOW} version ${CYAN}'$version'${NC}"
...
patch() {
  local apk_name=$1
  local apk_out=$2
  echo -e "${YELLOW}Starting patch ${RED}$apk_out${YELLOW}...${NC}"
